...
mutual
  ...
  data tj (Γ : Context) : Type Γ → Set where
    ...
    app   : ∀ 
      ...
      {A : γ ∶ Γ ⊢ *}   {B : γ ∶ Γ ⊢ (a ∶ ⟦ A ⟧ γ ⇒ *)} →   γ ∶ Γ ⊢ π (⟦ A ⟧ γ) (λ a → ⟦ B ⟧ (γ , a)) →   ∀ (a : γ ∶ Γ ⊢ ⟦ A ⟧ γ) →
    ---------------------------------------------------------------------------------------------------------------------------------
                                     γ ∶ Γ ⊢ ⟦ B ⟧ (γ , (⟦ a ⟧ γ))
    ...
    π₁    : ∀ 
      ...
      {A : γ ∶ Γ ⊢ *}     {B : γ ∶ Γ ⊢ (a ∶ ⟦ A ⟧ γ ⇒ *)} →      γ ∶ Γ ⊢ σ (⟦ A ⟧ γ) (λ a → ⟦ B ⟧ (γ , a)) →
    ----------------------------------------------------------------------------------------------------------
                                        γ ∶ Γ ⊢ ⟦ A ⟧ γ

    π₂    : ∀ 

      {A : γ ∶ Γ ⊢ *}        {B : γ ∶ Γ ⊢ (a ∶ ⟦ A ⟧ γ ⇒ *)}     (p : γ ∶ Γ ⊢ σ (⟦ A ⟧ γ) (λ a → ⟦ B ⟧ (γ , a))) → 
    ---------------------------------------------------------------------------------------------------------------
                      γ ∶ Γ ⊢ ⟦ B ⟧ (γ , (proj₁ (⟦ p ⟧ γ)))
    ...
    **    : 
    ...
    ---------------------------------------------------------------------------------------------------------------------------------------------------------------
                                  γ ∶ Γ ⊢ π (⟦ A ⟧ γ) (λ a → ⟦ B ⟧ (γ , a)) ≃ π (⟦ A' ⟧ γ) (λ a → ⟦ B' ⟧ (γ , a))
    ...
    sigmastar : ∀ 
      ...
      (A* : γ ∶ Γ ⊢ ⟦ A ⟧ γ ≃ ⟦ A' ⟧ γ) →    (γ ∶ Γ ⊢ (a ∶ ⟦ A ⟧ γ ⇒ (a' ∶ ⟦ A' ⟧ γ ⇒ (a* ∶ a ∼〈 ⟦ A* ⟧ γ 〉 a' ⇒ ⟦ B ⟧ (γ , a) ≃ ⟦ B' ⟧ (γ , a'))))) →
    ---------------------------------------------------------------------------------------------------------------------------------------------------------------
                                  γ ∶ Γ ⊢ σ (⟦ A ⟧ γ) (λ a → ⟦ B ⟧ (γ , a)) ≃ σ (⟦ A' ⟧ γ) (λ a → ⟦ B' ⟧ (γ , a))
  ...
  ⟦ var x ⟧ γ  = ⟦ x ⟧V γ
  ⟦ star ⟧ _ = *
  ⟦ pi A B ⟧ γ = π (⟦ A ⟧ γ) (λ a → ⟦ B ⟧ (γ , a))
  ⟦ sigma A B ⟧ γ = σ (⟦ A ⟧ γ) (λ a → ⟦ B ⟧ (γ , a))
  ⟦ eq A B ⟧ γ = ⟦ A ⟧ γ ≃ ⟦ B ⟧ γ
  ⟦ ∼ {A} {B} e a b ⟧ γ = ⟦ a ⟧ γ ∼〈 ⟦ e ⟧ γ 〉 ⟦ b ⟧ γ
  ⟦ Λ M ⟧ γ = λ a → ⟦ M ⟧ (γ , a)
  ⟦ app M N ⟧ γ = ⟦ M ⟧ γ (⟦ N ⟧ γ)
  ⟦ pair a b ⟧ γ = (⟦ a ⟧ γ) , (⟦ b ⟧ γ)
  ⟦ π₁ p ⟧ γ = proj₁ (⟦ p ⟧ γ)
